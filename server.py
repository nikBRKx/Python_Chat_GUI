...
def write_history(message, nickname): 
    ...
    try: 
        ...
        print('[INFO] Could not write chat history')
        ...
        try:
            ...
            if msg[0:9] == "/private ":
                ...
                if username in clients:
                    ...
                    client.send("[INFO] Username not valid.".encode('utf-8'))  
            elif msg == '!help':
                client.send('[INFO] To send a private message please enter: /private username \n'.encode('utf-8'))
                client.send('[INFO] To see users currently online please enter: !users \n'.encode('utf-8'))
            elif msg == '!users':
                #get current users
                users = clients.keys()
                client.send(f'[INFO] Currently online in chat: '.encode('utf-8'))
                for user in users:
                    client.send(f'{user} \n'.encode('utf-8'))
            else:
...
def receive():
    while True:
        ...
        print('[INFO] Connected with '+ str(address))
        ...
        #print(clients)
        print(f'[INFO] Nickname  of client {address[1]} is {nickname}')
        broadcast_admin(f'[INFO] {nickname} joined the chat!')
        ...
        try:
            ...
            print(f'[INFO] New history.txt created...')
        ...
        #send commands
        client.send('[INFO] To see commands please enter: !help \n'.encode('utf-8'))
...
#server start
print(f'[INFO] Server started... Searching for clients...')
receive()